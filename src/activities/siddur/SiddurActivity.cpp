...
#include "content/EdotWeekdayShacharit.h"
#include "fontIds.h"
  ...
  while (text[pos] != '\0' && static_cast<int>(page.lines.size()) < kMaxPrayerLines) {
    if (text[pos] == '\n') {
      if (!currentLine.empty()) {
        ...
        currentLine.clear();
        currentLine.reserve(192);
      } else if (static_cast<int>(page.lines.size()) < kMaxPrayerLines) {
    ...
    std::string word(text + wordStart, pos - wordStart);
    ...
    if (renderer.getTextWidth(SIDDUR_HEBREW_16_FONT_ID, word.c_str(), EpdFontFamily::REGULAR,
                              BidiUtils::BidiBaseDir::RTL) <= maxWidth) {
      currentLine = std::move(word);
    } else {
      page.lines.push_back(renderer.truncatedText(SIDDUR_HEBREW_16_FONT_ID, word.c_str(), maxWidth));
